...
response = requests.get("http://localhost:5000/api/scores")   # actual HTTP call to your own server
...
for bar, value in zip(bars, df["score"]):      # writes the number above each bar
    plt.text(bar.get_x() + bar.get_width() / 2, value + 1, str(value), ha="center", fontsize=9)
